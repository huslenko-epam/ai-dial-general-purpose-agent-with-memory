...
class LongTermMemoryStore:
    ...
    async def _load_memories(self, api_key: str) -> MemoryCollection:
        ...
        dial_client = AsyncDial(
            base_url=self.endpoint, api_key=api_key, api_version="2025-01-01-preview"
        )
    ...
    async def add_memory(
        ...
        return f"Content has been successfully stored: {content}"
    ...
    async def search_memories(
        ...
        embeddings = np.array([m.embedding for m in collection.memories]).astype(
            "float32"
        )
        ...
        _, indices = index.search(normalized_query, k)
    ...
    def _needs_deduplication(self, collection: MemoryCollection) -> bool:
        ...
        memories_count = len(collection.memories)

        try:
            if collection.last_deduplicated_at is None and memories_count > 10:
                return True

            time_since_dedup = datetime.now(UTC) - collection.last_deduplicated_at
            return (
                time_since_dedup > timedelta(hours=self.DEDUP_INTERVAL_HOURS)
                and memories_count > 10
            )
        except Exception as e:
    ...
    def _deduplicate_fast(self, memories: list[Memory]) -> list[Memory]:
        ...
        deduplicated = [
            m for i, m in enumerate(memories) if i not in duplicates_to_remove
        ]
        return deduplicated
    ...
    async def delete_all_memories(
        ...
        try:
            ...
            print(f"Error when trying to delete memory file: {e}")
